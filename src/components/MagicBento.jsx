...
const DEFAULT_GLOW_COLOR = '210, 140, 60'; // Maidan amber
...
const MagicCard = ({
    ...
    }, [animateParticles, clearAllParticles, enableParticles, enableTilt, enableMagnetism, clickEffect, glowColor]);

    const baseClass = `magic-card ${enableGlow ? 'magic-card--glow' : ''} ${className}`;

    return (
        <div ref={cardRef} className={baseClass} style={{ position: 'relative', overflow: 'hidden' }}>
            {children}
        </div>
    );
};
...
const GlobalSpotlight = ({
    ...
    `;
        ...
        const handleMouseMove = e => {
            ...
            const section = containerRef.current.closest('.magic-grid-section');
...
const MagicBento = ({
    ...
    return (
        <>
            {enableSpotlight && <GlobalSpotlight containerRef={gridRef} enabled={enableSpotlight} spotlightRadius={spotlightRadius} glowColor={glowColor} />}

            <div ref={gridRef} className={`magic-grid ${gridClassName}`}>
                {items.map((item, idx) => (
                    <MagicCard
                        key={idx}
